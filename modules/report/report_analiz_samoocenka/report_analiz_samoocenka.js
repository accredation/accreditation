...
function preperaReport(){
    ...
    if(flag_yur_lica_value){
        reportYurLica(oblast_value,oblast_text, status_value,status_text, dateAccept_value, dateComplete_value, typeOrg_value,typeOrg_text, criteriaAll_value,criteriaAll_text, arrCriteriaId_report_analiz_samoocenka.toString())
    } else {
        reportWithOutYurLica(oblast_value,oblast_text, status_value,status_text, dateAccept_value, dateComplete_value, typeOrg_value,typeOrg_text, criteriaAll_value,criteriaAll_text, arrCriteriaId_report_analiz_samoocenka.toString())
    }
    ...
    $.ajax({
        ...
        //: регион "` + value1 +`", со статусом "` + value2 + `", в период с `+  new Date(date1.value).toLocaleDateString() +` по ` +  new Date(date2.value).toLocaleDateString() +`; тип организации "` + value3 +`"`;
         ...
         divReportUsl.innerHTML = divReportUsl.innerHTML + '<b>' +' Статус: '+'</b>' + status_text + '<br/>'
           ...
           if((criteriaAll_value == 1) && (arrCriteriaStr_report_analiz_samoocenka.length>0)) {
                let arr =  arrCriteriaStr_report_analiz_samoocenka.map(item=>{
