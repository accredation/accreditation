...
if (isset($_FILES['prikazNaznach']['name'])) {
    $file_name = $_FILES['prikazNaznach']['name'];
    $file_tmp = $_FILES['prikazNaznach']['tmp_name'];

    move_uploaded_file($file_tmp, "../docs/documents/" . $login.'/'.$id_application . "/" . $file_name);

    $insertquery =
        "UPDATE applications set prikazNaznach = '$file_name' where id_application='$new_id_application'";
