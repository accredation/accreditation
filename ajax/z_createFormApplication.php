...
<br>
    ...
    left: 9%;
    text-align: justify;
">свидетельство о соответствии государственной организации здравоохранения базовым критериям медицинской аккредитации по видам оказываемой медицинской помощи по профилям заболеваний, состояниям, синдромам:</div>
...
while ($row_subs = mysqli_fetch_assoc($result_subs)) {
    ...
    $query_tds = "select tt.name as namet, ltc.name_profile as namel, dep.name as named, level FROM z_department AS dep
                        ...
                        where dep.id_subvision = '$id_sub' order by level";
